...
void read_serial_data(void)
{
	...
	if(sys_flag.LPWater_Value >= Sys_Admin.ChaYa_WaterLow_Set)
		{
			IO_Status.Target.water_protect = OK;
		}

	if(sys_flag.LPWater_Value >= Sys_Admin.ChaYa_WaterMid_Set)
		{
			IO_Status.Target.water_protect = OK;
			IO_Status.Target.water_mid = OK;
		}

	if(sys_flag.LPWater_Value >= Sys_Admin.ChaYa_WaterHigh_Set)
		{
			IO_Status.Target.water_protect = OK;
			IO_Status.Target.water_mid = OK;
	...
	if(sys_flag.LPWater_Value >= 1200)  //600mm	则认为水位有问题，需要检查
		{
			IO_Status.Target.water_protect = OK;
			IO_Status.Target.water_mid = FALSE;
			IO_Status.Target.water_high = OK;
		}
