...
uint8  SpiReadData(void)
{
	...
	if(sys_flag.Address_Number == 0)  //总控不检查本体的温度
		{
			Sys_Admin.YuRe_Enabled = 0; //总控不报警，不驱动本体温度保护
		}
			...
			if(sys_flag.Protect_WenDu >= Sys_Admin.Inside_WenDu_ProtectValue)
				{
					...
					if(Alarm_Count >= 4)  //防止温度瞬跳，增加了滤波
						{
							if(sys_flag.Protect_WenDu >= 900)
								{
									//本体温度传感器未有效连接
									sys_flag.Error_Code = Error6_BentiWenDu_Unconnect;
								}
							else
								{
									//超过保护值
