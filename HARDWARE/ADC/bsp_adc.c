...

{
	...
	if(Sys_Admin.DeviceMaxPressureSet <= 160)
		{
			PID.Real_Value = (pressure - 559) * 100/140;   //精确到0.001
	...
	if(Sys_Admin.DeviceMaxPressureSet > 160  && Sys_Admin.DeviceMaxPressureSet <= 250)
		{
			PID.Real_Value = (pressure - 559)* 250/224 ;//精确到0.001
		...
		if(sys_flag.ADC_100msFlag)
			{
						...
						if(Temperature_Data.Smoke_Tem > 100)
							{
								...
								if(Temperature_Data.Smoke_Tem > 9000)
									{
										Temperature_Data.Smoke_Tem = 9999;
									}
				...
				LCD4013X.DLCD.Inside_High_Pressure = ResData / 100; //换算成小数
