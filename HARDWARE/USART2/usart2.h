...
typedef struct __attribute__((packed)) _CMD_UANIS
{
	...
	uint16  Mode_Index;
	uint16   AliveOK_Numbers; //设备正常在线数量，有故障的除外
	
	uint16  PID_Next_Time;
	uint16  PID_Pvalue;
	uint16  PID_Ivalue;
	uint16  PID_Dvalue;
	uint16  Union16_Flag; //16台设备允许使用标志
	...
	uint16  A8_WorkTime;
	uint16  A9_WorkTime;
	uint16  A10_WorkTime;  //60

	uint16  Error_Reset; //故障复位
	uint16  Alarm_OFF; //报警声音关闭

	uint16  Devive_Style;  //设备类型
	uint16  Max_Address;  //允许最大的机组数量
	uint16  Sys_Version; //系统版本号
	uint16  ModBus_Address;
	uint16  OFFlive_Numbers; //禁止在线数量


	uint16  PaiYan_WenDu; //排烟温度
	uint16  PaiYan_AlarmValue; //排烟温度报警值
	uint16  Union_Error; //总控故障码
	uint16  Alarm_Allow_Flag;  //外部报警允许标志集合
	uint16  ZongKong_RelaysOut;

	uint16  Steam_Speed;  //蒸汽瞬时流量
	uint16  LL_Steam;  //LL累计流量
	uint16  HH_Steam;  //HH累计流量
	uint16  Data14;
	uint16  Data15; 

	uint16  Data16;    //共96字节
	uint16  Data17;
	uint16  HPWater_Value_A1;
	uint16  HPWater_Value_A2;
	uint16  HPWater_Value_A3;   //共104

	uint16  HPWater_Value_A4;
	uint16  LPWater_Value_A1;
	uint16  LPWater_Value_A2;
	uint16  LPWater_Value_A3;
	uint16  LPWater_Value_A4;  //共114
...
typedef struct __attribute__((packed)) _LCD4013JZ_UNIS
{
	...
	uint16  UnionControl_Flag;  //是否被联控的标志
	uint16  Device_State;  //设备当前状态
	uint16  Error_Code;
	uint16  Air_Power; //10个字节
	
	uint16  Flame_State;
	uint16  Pump_State;   
	uint16  Water_State;
	uint16  Paiwu_State;
	uint16  Air_State;//20个字节

	
	uint16  lianxuFa_State;
	uint16  Air_Speed;   
	uint16  First_Blow_Time; //用不到
	uint16  Last_Blow_Time;//用不到
	uint16  Dian_Huo_Power;//30个字节
	
	uint16  Max_Work_Power;
	uint16  Danger_Smoke_Value;   //用不到
	uint16  YuRe_Enabled;  // 用不到
	uint16  Inside_WenDu_ProtectValue; //本体温度保护值   
	uint16  Inside_WenDu;  //内部实际温度  //40个字节
	
	uint16  Work_Time;  //累计运行时间
	uint16  InPut_Data; //最多16位IO口数据  
	uint16  Relays_Out;  //继电器输出
	uint16  HPWater_Value;
	uint16  LPWater_Value;   //50个字节
	
	uint16  ChaYa_WaterLow_Set; 
	uint16  ChaYa_WaterMid_Set;
	uint16  ChaYa_WaterHigh_Set;
	uint16  InsideWater_AlarmValue;
	uint16  Data11;  // 60

	uint16  Data12;
	uint16  Data13;
	uint16  Address;  // 66
	uint16  Soft_Version;  // 68  软件版本
...
/**********************************************************/
	...
	float   Smoke_WenDu;
	float   LuNei_WenDu;
	float   Target_Pressure;//88个字节
	
	float   Stop_Pressure;  
	float   Start_Pressure;
	float   Max_Pressure;  //额定压力//100个字节	
...
typedef union  _LCD4013_MsESS
{  //
	uint8 Datas[100]; //必须要能够被4整除，防浮点数
...
extern ALCD10Struct UnionLCD;
extern UNION_GGA AUnionD;
...
extern LCD4013_Struct LCD4013X;

extern LCD10K1_Struct LCD10X1;
...
uint8 LCD4013_MmodBus2_Communicastion(void);
