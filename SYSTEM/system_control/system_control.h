...
enum
{
 ...
 enum
 {
	 ...
	 Error2_YaBianProtect,	//2  压力变送器超过保护值
	 Error3_LowGas,//3 燃气低保护
	 Error4_YaBianLoss, //4 压力变送器断线，未连接好
	 Error5_LowWater,//5极低水位保护,
	 Error6_BentiWenDu_Unconnect,//6本体温度传感器未有效连接
	 Error7_FlameZiJian,//7火焰自检
	 Error8_WaterLogic,//8水位探针逻辑故障
	 Error9_AirPressureBad,//9 风压异常
	 Error10_SteamValueBad,//10 未使用
	 Error11_DianHuo_Bad,//11点火失败
	 Error12_FlameLose, //12运行中火焰熄灭
	 Error13_AirControlFail, //13 风机转速异常
	 Error14_BenTiValueVBad,//14 本体温度超过保护值
	 Error15_RebaoBad,  //15 本体过热保护
	 Error16_SmokeValueHigh,//16 烟气温度超过保护值
	 Error17_OutWenKong_TxBad,// 17 安全通信超时
	 Error18_SupplyWater_Error,//18 进水阀故障
	 Error19_SupplyWater_UNtalk,//19 进水阀通信丢失
	 Error20_XB_HighPressureYabian_Bad,//20 相变高压侧压力变送器超过保护值
	 Error21_XB_HighPressureYAKONG_Bad,//21 相变高压侧机械压控超压！！
	 Error22_XB_HighPressureWater_Low, //22高压侧低水位
	 unused// 23 未使用
...
volatile	uint8 XB_WaterLow_Flag;
	...
	uint16 HPWater_Value;  //高压侧液位值
	uint16 LPWater_Value;  //低压侧液位值

	volatile	uint8 A3B3_ChaYaMokuai_TxRX_Count;

	uint8 PowerOn_Index;
	uint8 LCD10_Connect;
	uint8 Already_WorkNumbers;
	uint8 Device_ErrorNumbers;
	uint8 PaiYanAlarm_Flag;  //排烟报警允许标志

	uint16 Pingjun_Power; //平均的燃烧功率
	

	uint8 Work_1S_Flag;
	uint16 AirWork_Time ;
	uint8 YanDao_FengFa_Index;
	uint8 Pid_First_Start;
...
typedef struct UPIDSS //系统累计时间结构体
{
		...
		unsigned int LastValue; // Error[-1] 
		unsigned int PrevValue; // Error[-2] 
		unsigned int SumError; // Sums of Errors 

		uint16   Real_Value;
		uint8  Flag_100ms;
		uint16 Unchange_Count100ms;  //值未改变计数
		uint8  Up_Flag;
		uint8  Down_Flag;
		uint8  Down_Time;
		uint8  Change_Speed;  //  0.001Mpa / M 0.1s
		uint8  Next_Time; 


		uint32 Out_Put ;  //1000 = 100% 

		uint32 Old_Put;  //上次PID输出的值
